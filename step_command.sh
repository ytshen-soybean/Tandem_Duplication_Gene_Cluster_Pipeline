第一步：蛋白序列all-all的比对
##每个基因选择primary_protein，可以通过jcvi自带的format工具去选择，也可以根据自己的策略去选择。利用blastp或者diamond进行蛋白之间的比对，比对时不设置每个序列的最多比对数目，仅设置--evalue。
##根据严格程度，也可以设置比对相似度，coverage百分比（--id 80 --query-cover 80 --subject-cover 80）等参数
#ZH13V3.0_primary_protein.fasta来自/home/ytshen/ytshen/ytshen/T2T/tandem_repeat/20250225/DupGene_finder/prepare
diamond makedb --in ZH13V3.0_primary_protein.fasta -d ZH13V3.0
diamond blastp -d ZH13V3.0 -q ZH13V3.0_primary_protein.fasta -o Soy.blast -p 40 --sensitive --evalue 1e-10 --quiet   （691004行）
##整理blast结果
#过滤自身与自身的比对
